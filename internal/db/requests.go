...
func (q *Queries) CreateRequest(ctx context.Context, req *Request) error {
	_, err := q.pool.Exec(ctx,
		`INSERT INTO request (endpoint_id, seq, method, path, query_params, headers, body, body_size, content_type, source_ip, response_status, response_body)
...
func (q *Queries) ListRequests(ctx context.Context, endpointID int64, limit int) ([]Request, error) {
	rows, err := q.pool.Query(ctx,
		`SELECT id, endpoint_id, seq, method, path, query_params, headers, body, body_size, content_type, source_ip, response_status, response_body, received_at
		 FROM request WHERE endpoint_id = $1 ORDER BY received_at DESC LIMIT $2`,
		endpointID, limit,
	)
	if err != nil {
		return nil, err
...
func (q *Queries) GetRequest(ctx context.Context, endpointID int64, seq int) (*Request, error) {
	r := &Request{}
	err := q.pool.QueryRow(ctx,
		`SELECT id, endpoint_id, seq, method, path, query_params, headers, body, body_size, content_type, source_ip, response_status, response_body, received_at
		 FROM request WHERE endpoint_id = $1 AND seq = $2`,
		endpointID, seq,
...
func (q *Queries) TrimRequests(ctx context.Context, endpointID int64, keepCount int) error {
	_, err := q.pool.Exec(ctx,
		`DELETE FROM request WHERE endpoint_id = $1 AND id NOT IN (
			SELECT id FROM request WHERE endpoint_id = $1 ORDER BY received_at DESC LIMIT $2
		)`,
